...
class PurePursuitController(Node):
    ...
    def publish_cmd_vel(self, target_point):
        ...
        #VESC related commands
        linear_velocity = self.linear_velocity
        angular_velocity = self.calculate_angular_velocity(heading_error)
        throttle = self.map_linear_velocity_to_duty_cycle(linear_velocity)
        steering_angle = self.map_angular_velocity_to_steering(angular_velocity)

        if self.current_path_index == 0 and abs(heading_error) > math.pi/6:
            self.get_logger().info("Aligning with path...")
	    throttle = 0.0
	    steering_angle = math.copysign(self.max_angular_velocity, heading_error)
	    self.publish_vesc_commands(throttle, steering_angle)
            #cmd_vel = Twist()
            #cmd_vel.linear.x = 0.0
	    ...
	    throttle = self.map_linear_velocity_to_duty_cycle(self.linear_velocity)
	    steering_angle = self.map_angular_velocty_to_steering(heading_error)
